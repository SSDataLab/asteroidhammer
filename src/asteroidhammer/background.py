...
_keys = ['fnames', 'nfiles', 'nchunks', 'npixels', 'magnitude_limit', 'testframe', 'camera', 'ccd', 'sector',
        'wcs', 'locs', 'dra', 'ddec', 'phot', 'mag', 'star_aper',
        'mask', 'bright_mask', 'bkg_aper', 'avg', 'w', 'dra_bin', 'ddec_bin', 'med', 'dmed']
_keys_lite = ['fnames', 'nfiles', 'nchunks', 'npixels', 'magnitude_limit', 'testframe', 'camera', 'ccd', 'sector', 'w', 'dra_bin', 'ddec_bin', 'med', 'dmed']
...
class BackgroundCorrector(object):
    ...
    def __init__(self, fnames=None, nchunks=16, testframe=None, magnitude_limit=16):
        """Parameters
    ...
    def find_bkg_solution(self, nbins=(30, 30)):
        ...
        fe = fitsio.read(self.fnames[self.testframe], ext=2)[:2048, 45:2048+45].ravel()
        ...
        sigma_w_inv = self.Xf.T[:, mask].dot(self.Xf[mask]).toarray() + np.diag(1/prior_sigma**2)
        ...
        mask &= (chi < np.percentile(chi, 99.9))
...
#        self.avg /= self.nfiles
    ...
    def save(self, out=None, lite=False):
        if out is None:
    ...
    def get_full_bkg_frame(self, frames=0):
        ...
        model =  np.asarray([self.Xf.dot(self.w[frame]).reshape((2048, 2048)) for frame in frames])
        if len(frames) == 1:
            return model[0]
        return model
    ...
    def _get_X(self, c, r):
        ...
        Xrf = lk.designmatrix.create_spline_matrix(r, knots=list(np.linspace(0, 2048, 82)[1:-1]), degree=3).append_constant().to_sparse().X.tocsr()
